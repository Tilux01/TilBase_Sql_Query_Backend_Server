export const fetchHistory = async (connection, user_id, project_id, page = 1) => {
    ...
    try {
        const [result] = await connection.query(command, value)
        return result
    } catch (error) {
        console.log(error);
        return []
...
export const checkPlan = async (connection, userId, ProjectPlan) => {
    try {
        ...
        const [executeCheckPlan] = await connection.query(checkPlanCommand, checkPlanValue)
...
export const createProjectTable = async(connection, userId, ProjectName, projectDescription, Environment, ProjectKey, serverName, serverRegion, ProjectType, ProjectPlan)=>{
    try {
        ...
        const [ProjectResult] = await connection.query(saveProject, Values)
...
export const planProjectAdd = async(connection, userId) =>{
    try {
        ...
        const value = [userId]
        const [updatePlan] = await connection.query(getPlanCommand, value)
        ...
        try {
            ...
            const [saveProject] = await connection.query(addProject, addValue)
        } catch (error) {
            console.log(error);
...
export const connectionAuth = async(connection, userId, user_key, ProjectId, project_key, clusterKey, clusterPassword) =>{
    ...
    const [checkCluster] = await connection.query(checkClusterCommand, checkClusterValue)
