...
const getCollections = async (req, res) => {
    ...
    try {
        ...
        let query = "SELECT DISTINCT parent_path FROM Document_Store WHERE cluster_id = ?";
...
const getDocuments = async (req, res) => {
    ...
    try {
        ...
        const query = "SELECT id, path, created_at FROM Document_Store WHERE cluster_id = ? AND parent_path = ?";
...
const getDocumentData = async (req, res) => {
    ...
    try {
        const connection = await makeConnection();
        const query = "SELECT document_data FROM Document_Store WHERE cluster_id = ? AND path = ?";
...
const setDocumentData = async (req, res) => {
    ...
    try {
        ...
        if (existing.length > 0) {
            oldSize = Buffer.byteLength(existing[0].document_data, 'utf8');
        }
...
const deleteDocument = async (req, res) => {
    ...
    try {
        ...
        const [docs] = await connection.query("SELECT document_data FROM Document_Store WHERE cluster_id = ? AND (path = ? OR path LIKE ?)", [clusterId, path, `${path}/%`]);
        let sizeDelta = 0;
        docs.forEach(doc => {
            sizeDelta += Buffer.byteLength(doc.document_data, 'utf8');
        });

        const query = "DELETE FROM Document_Store WHERE cluster_id = ? AND (path = ? OR path LIKE ?)";
...
const updateDocument = async (req, res) => {
    ...
    try {
        const connection = await makeConnection();
        const [rows] = await connection.query("SELECT document_data FROM Document_Store WHERE cluster_id = ? AND path = ?", [clusterId, path]);
        let existing = {};
        if (rows.length > 0) {
        ...
        const oldSize = rows.length > 0 ? Buffer.byteLength(rows[0].document_data, 'utf8') : 0;
        const newSize = Buffer.byteLength(jsonData, 'utf8');
        const sizeDelta = newSize - oldSize;
...
const findDocuments = async (req, res) => {
    ...
    try {
        const connection = await makeConnection();
        const [rows] = await connection.query("SELECT id, path, document_data, created_at FROM Document_Store WHERE cluster_id = ? AND parent_path = ?", [clusterId, parentPath]);
...
const bulkSaveDocuments = async (req, res) => {
    ...
    try {
        ...
        let oldSize = 0;
        if (paths.length > 0) {
            const [existing] = await connection.query(`SELECT document_data FROM Document_Store WHERE cluster_id = ? AND path IN (${placeholders})`, [clusterId, ...paths]);
            existing.forEach(row => { oldSize += Buffer.byteLength(row.document_data, 'utf8'); });
        }
        ...
        try {
            for (const op of operations) {
                ...
                if (type === 'save') {
                    ...
                    const [existing] = await connection.query("SELECT document_data FROM Document_Store WHERE cluster_id = ? AND path = ?", [clusterId, path]);
                    if (existing.length > 0) sizeDelta -= Buffer.byteLength(existing[0].document_data, 'utf8');
                    ...
                    let existing = {};
                    if (rows.length > 0) {
                        sizeDelta -= Buffer.byteLength(rows[0].document_data, 'utf8');
                        existing = typeof rows[0].document_data === 'string' ? JSON.parse(rows[0].document_data) : rows[0].document_data;
                    }
                    ...
                    const [docs] = await connection.query("SELECT document_data FROM Document_Store WHERE cluster_id = ? AND (path = ? OR path LIKE ?)", [clusterId, path, `${path}/%`]);
                    docs.forEach(doc => { sizeDelta -= Buffer.byteLength(doc.document_data, 'utf8'); });
